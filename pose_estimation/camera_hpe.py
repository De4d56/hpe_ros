...
def main(data):
    img = numpy.frombuffer(data.data, dtype=numpy.uint8).reshape(data.height, data.width, -1)
    img = torch.from_numpy(img)
    # Data loading code
    normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                     std=[0.229, 0.224, 0.225])
    #valid_data = eval('dataset.'+config.DATASET.DATASET)(
    #    config,
    #    config.DATASET.ROOT,
    ...
    network_use(config, valid_loader, model, criterion)
